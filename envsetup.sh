...
function print_lunch_menu()
{
    ...
    if [ "z${INVICTRIX_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Invictrix model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch invictrix_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Invictrix GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/invictrix/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/invictrix/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $INVICTRIX_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.invictrix.device | grep -q "$INVICTRIX_BUILD");
    then
        ...
        echo "The connected device does not appear to be $INVICTRIX_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.invictrix.device | grep -q "$INVICTRIX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/invictrix/build/envsetup.sh
